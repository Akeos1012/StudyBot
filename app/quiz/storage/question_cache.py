...
from ..metadata.question_metadata import create_metadata

from ..validation.question_validator import is_valid_question
...
class QuestionCache:
    ...
    def get_question_by_id(
        ...
        """
        Find a question across all cache pools by question_id.
        """

        for key, pool in self.cache.items():
    ...
    def update_question(
        ...
        """
        Replace an existing question in cache.
        """
    ...
    def _adaptive_score(
        ...
        # Prefer weak questions
        success_rate = metadata.get(
            "success_rate",
            0.0
        )

        score += (1 - success_rate) * 5

        # Prefer questions answered incorrectly
        times_wrong = metadata.get(
            "times_wrong",
            0
        )

        score += times_wrong * 0.5
    ...
    def sample(
        ...
        if exclude_ids:
            pool = [q for q in pool if q.get("question_id") not in exclude_ids]

        if not pool:
            logger.info("Pool empty or exhausted")
            return None
        ...
        # Take top candidates (e.g., top 3x requested count, or the whole pool if smaller)
        candidate_count = min(len(ranked_pool), count * 3)
        candidates = ranked_pool[:candidate_count]

        # Randomly sample from top candidates to introduce variation
        sample_count = min(len(candidates), count)
