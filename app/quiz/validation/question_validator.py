...
from ..utils.question_constants import MAX_QUESTION_LENGTH
...
def validate_distractors(question: dict, metrics_context: MetricsContext = None) -> bool:
    ...
    for option in question.get("options", []):
        ...
        for bad in BAD_DISTRACTOR_PATTERNS:
            if bad in text:
                _log_failure(
                    question, "distractor", f"Placeholder distractor detected: {text}", metrics_context
                )
                return False
    return True
...
def validate_structure(question: dict, metrics_context: MetricsContext = None) -> bool:
    ...
    if missing:
        _log_failure(
            question, "structure", f"Missing required fields: {missing}", metrics_context
        )
        return False
    ...
    if not is_fill_blank:
        if not isinstance(question["options"], list) or len(question["options"]) != 4:
            _log_failure(
                question, "structure", "Options must contain exactly 4 items", metrics_context
            )
            return False
    ...
    if re.search(r"\b[A-D]\)", q_text):
        _log_failure(
            question, "structure", "Question contains leaked option markers", metrics_context
        )
        return False
...
def normalize_and_validate_correct_field(question: dict, metrics_context: MetricsContext = None) -> bool:
    ...
    _log_failure(question, "correct_field", "Unable to resolve correct answer", metrics_context)
    return False
...
def validate_question_focus(
    ...
    if "layer" in q_text.lower() and "layer" not in concept_lower:
        _log_failure(
            question, "focus", f"Question uses 'layer' but concept is '{concept}'", metrics_context
        )
        return False
    ...
    _log_failure(
        question, "focus", f"Question doesn't reference concept '{concept}'", metrics_context
    )
    return False
...
def validate_question_uniqueness(question: dict, metrics_context: MetricsContext = None) -> bool:
    """Reject questions where a distractor is described by the question."""
    question_text = question.get("question", "").lower()
    question_type = question.get("type", "mcq")

    if question_type == "fill_blank":
        return validate_fill_blank_question(question, metrics_context)
...
def validate_fill_blank_question(question: dict, metrics_context: MetricsContext = None) -> bool:
    ...
    if text.count("_______") != 1:
        _log_failure(question, "fill_blank", "Must contain exactly one blank", metrics_context)
        return False

    # Full answer should not appear in question
    if correct.lower() in text.lower():
        _log_failure(question, "fill_blank", "Answer appears inside question", metrics_context)
        return False
    ...
    for pattern in BANNED_FILL_BLANK_PATTERNS:
        if pattern in lower:
            _log_failure(question, "fill_blank", f"Contains banned pattern: {pattern}", metrics_context)
            return False
