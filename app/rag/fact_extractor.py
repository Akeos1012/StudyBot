...
from ..models.fact_schema import create_fact, is_weak_concept, detect_concept_type, ConceptType
...
class HeadingFilter:
    ...
    ORGANIZATIONAL_SUFFIXES = {
        ...
        "examples", "notes", "references", "further reading",
        "analysis", "level", "requirements", "features",
        "workflow", "implementation", "discussion",
        "classification", "categories", "types", "architecture"
    }
    
    CONCEPT_PREDICATES = {
        "is", "are", "refers to", "means", "stands for",
        "allows", "enables", "provides", "stores",
        "manages", "reduces", "improves", "uses",
        "supports", "offers", "helps", "contains", "includes"
    }
    ...
    def is_structural(self, line: str, context: str = "") -> Tuple[bool, str]:
        ...
        if len(words) <= 4 and not any(p in heading_lower for p in HeadingFilter.CONCEPT_PREDICATES):
            return True, "title_only_heading"
        
        structural_markers = {'overview', 'summary', 'introduction', 'conclusion', 'analysis'}
...
class SemanticConceptExtractor:
    ...
    VERB_STARTS = {
        "allows", "enabling", "enables", "provides", "providing",
        "stores", "storing", "manages", "managing", "reduces", "reducing",
        "improves", "improving", "uses", "using", "supports", "supporting",
        "offers", "offering", "helps", "helping", "contains", "containing",
        "includes", "including", "focuses", "focusing", "creates", "creating",
        "runs", "running", "processes", "processing", "handles", "handling",
        "generates", "generating", "builds", "building", "works", "working",
        "makes", "making", "takes", "taking", "gives", "giving", "shows", "showing",
        "does", "doing", "performs", "performing", "executes", "executing"
    }
    
    # Filler words that should never start a concept
    FILLER_STARTS = {
        "the", "this", "these", "those", "where", "when",
        "why", "how", "using", "used", "because", "since",
        "during", "after", "before", "through", "without",
        "with", "for", "from", "into", "onto", "upon", "via",
        "between", "among", "within", "outside"
    }
    ...
    SECTION_LABELS = {
        ...
        "examples", "notes", "references", "architecture",
        "workflow", "implementation", "discussion",
        "analysis", "level", "foundation", "foundations",
        "requirements", "features", "classification",
        "categories", "types", "architecture"
    }
    ...
    VALID_ACRONYMS = {
        "CPU", "GPU", "RAM", "ROM", "BIOS", "USB", "HDMI", "PCI",
        "SSD", "HDD", "LAN", "WAN", "VPN", "DNS", "DHCP", "TCP",
        "UDP", "IP", "HTTP", "HTTPS", "FTP", "SSH", "SMTP", "POP3",
        "IMAP", "API", "SQL", "DBMS", "IoT", "ML", "AI", "NLP",
        "CNN", "RNN", "GPT", "BERT", "OOP", "ACID", "JSON", "XML",
        "HTML", "CSS", "JS", "PHP", "C++", "C#", "REST", "SOAP"
    }
    ...
    def _is_canonical_concept(self, concept: str) -> bool:
        ...
        for word in words:
            if word in self.SECTION_LABELS and len(words) > 1:
                if concept not in self._concept_type_overrides:
                    logger.debug(f"Rejected '{concept}': contains section label '{word}'")
                    return False
        ...
        if first_word in {'where', 'when', 'why', 'how', 'what', 'which', 'who', 'whom'}:
            logger.debug(f"Rejected '{concept}': starts with question word '{first_word}'")
            return False
        ...
        vague_terms = {'system', 'process', 'method', 'approach', 'technique', 'mechanism', 'framework'}
        if len(words) == 1:
        ...
        if len(words) >= 2:
            for w in words:
                ...
                if not w[0].isupper() and w not in {'and', 'of', 'for', 'in', 'on', 'at', 'to', 'by', 'with', 'without'}:
    ...
    def _normalize_concept(self, concept: str) -> str:
        ...
        # Fix acronyms
        acronyms = {
            "Ai": "AI", "Cpu": "CPU", "Gpu": "GPU", "Sql": "SQL",
            "Api": "API", "Dbms": "DBMS", "Iot": "IoT", "Ml": "ML",
            "Nlp": "NLP", "Os": "OS", "Ram": "RAM", "Rom": "ROM",
            "Bios": "BIOS", "Usb": "USB", "Hdmi": "HDMI", "Pci": "PCI",
            "Ssd": "SSD", "Hdd": "HDD", "Lan": "LAN", "Wan": "WAN",
            "Vpn": "VPN", "Dns": "DNS", "Dhcp": "DHCP", "Tcp": "TCP",
            "Udp": "UDP", "Ip": "IP", "Http": "HTTP", "Https": "HTTPS",
            "Ftp": "FTP", "Ssh": "SSH", "Smtp": "SMTP", "Pop3": "POP3",
            "Imap": "IMAP",
        }
...
class ConceptValidator:
    ...
    def __init__(self):
        self.concept_type_overrides = {
            "Quick Sort": "algorithm", "Merge Sort": "algorithm",
            "Bubble Sort": "algorithm", "Binary Search": "algorithm",
            "Dynamic Programming": "algorithm", "Greedy": "algorithm",
            "Divide and Conquer": "algorithm", "Recursion": "algorithm",
            "Deep Learning": "model", "Neural Network": "model",
            "CNN": "model", "RNN": "model", "Transformer": "model",
            "Time Complexity": "metric", "Space Complexity": "metric",
            "Big O": "metric", "Array": "data_structure",
            "Linked List": "data_structure", "Stack": "data_structure",
            "Queue": "data_structure", "Tree": "data_structure",
            "Graph": "data_structure", "DBMS": "system",
            "Operating System": "system", "File System": "system",
            "Normalization": "process", "Backpropagation": "process",
        ...
        self.weak_single_words = {
            ...
            "resource", "component", "module", "architecture"
        }
...
class FactExtractor:
    ...
    def _build_atomic_fact(self, text: str, topic: str, source: str,
                           weight: int = 7) -> Tuple[Optional[Dict[str, Any]], Optional[RejectionInfo]]:
        ...
        if len(cleaned.split()) < 4:
            return None, RejectionInfo(RejectionReason.TOO_SHORT, "less_than_4_words", text)
        
        if any(marker in cleaned.lower() for marker in ['#', '[[', ']]', '---', 'http']):
        ...
        if not concept:
            return None, RejectionInfo(RejectionReason.NO_CONCEPT, "no_concept_extracted", text)
        ...
        try:
            concept_type = self.concept_type_overrides.get(concept, detect_concept_type(concept, cleaned))
    ...
    def _extract_atomic_facts(self, content: str, topic: str,
        ...
        for raw_line in content.splitlines():
            ...
            if rejection:
                if rejection.reason == RejectionReason.STRUCTURAL_HEADING:
                    ...
                    logger.debug(f"Skipped heading: {rejection.detail} | {rejection.line[:50]}...")
                elif rejection.reason == RejectionReason.INVALID_CONCEPT:
                    stats.skipped_invalid_concept += 1
                    logger.debug(f"Skipped invalid concept: {rejection.detail} | {rejection.line[:50]}...")
                else:
                    stats.skipped_other += 1
                    logger.debug(f"Skipped: {rejection.reason.value} | {rejection.line[:50]}...")
                continue
    ...
    def extract_facts(self, content: str, topic: str,
                      source: str = "inline") -> List[Dict[str, Any]]:
        ...
        for f in all_facts:
            ...
            if key in unique_facts:
                if f.get("weight", 0) > unique_facts[key].get("weight", 0):
                    unique_facts[key] = f
                elif len(f.get("supporting_fact", "")) > len(unique_facts[key].get("supporting_fact", "")):
                    unique_facts[key] = f
            else:
                unique_facts[key] = f
